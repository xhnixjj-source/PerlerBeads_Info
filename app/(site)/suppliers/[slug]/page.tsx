  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
      ...
      <header className="mb-8 rounded-2xl border border-ink-200 bg-white p-6 shadow-sm">
        ...
        <div
          className="mt-4 rounded-xl border border-dashed border-ink-200 bg-amber-50/40 px-3 py-2.5 text-xs text-ink-700"
          title="Illustrative numbers for the demo — not live analytics."
        >
          <span className="font-semibold text-ink-900">Sample directory activity (demo):</span>{" "}
          <span className="font-medium">~{demoSupplierInquiries30d(supplier.slug)}</span> buyer inquiries in the last 30
          days (sim.) · typical first response under{" "}
          <span className="font-medium">{demoSupplierResponseHours(supplier.slug)}h</span>
        </div>
      </header>
      ...
      <div className="grid gap-8 lg:grid-cols-[1.45fr_1fr]">
        ...
        <aside className="space-y-5 rounded-2xl border border-ink-200 bg-white p-6 shadow-sm lg:sticky lg:top-24 lg:h-fit">
          ...
          <div className="border-t border-ink-200 pt-5">
            ...
            <div className="mt-4">
              <InquiryForm source="supplier" supplierId={supplier.id} backLabel={m.inquiry.backSuppliers} />
            </div>
            <div className="mt-6">
