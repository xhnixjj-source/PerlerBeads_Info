...
import { parseJsonBody, orderUpdateSchema, uuidParam } from "@/lib/admin/schemas";
...
export async function GET(request: Request, context: Ctx) {
  ...
  const { data, error } = await supabase.from("orders").select("*").eq("id", idParse.data).maybeSingle();
  if (error) {
    console.error("[admin orders GET id]", error.message);
...
export async function PUT(request: Request, context: Ctx) {
  const auth = await requireAdminRequest();
  if (!auth.ok) return auth.response;
  if (!canAccess(auth.ctx.role, "orders", "PUT")) {
    return jsonError(403, "Forbidden", "forbidden");
  }
  const svc = requireServiceClient();
  if (!svc.ok) return svc.response;
  ...
  const parsed = parseJsonBody(raw, orderUpdateSchema);
  ...
  const { data: before, error: fetchErr } = await supabase
    ...
    .maybeSingle();
  if (fetchErr) {
    console.error("[admin orders PUT fetch]", fetchErr.message);
    return jsonError(500, fetchErr.message, "database");
  }
  if (!before) return jsonError(404, "Not found", "not_found");

  const { data, error } = await supabase.from("orders").update(patch).eq("id", idParse.data).select("*").single();
  if (error) {
    console.error("[admin orders PUT]", error.message);
    return jsonError(500, error.message, "database");
  }
