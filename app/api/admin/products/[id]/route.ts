...
import { parseJsonBody, productUpdateSchema, uuidParam } from "@/lib/admin/schemas";
...
export async function GET(request: Request, context: Ctx) {
  ...
  if (!canAccess(auth.ctx.role, "products", "GET")) {
...
export async function PUT(request: Request, context: Ctx) {
  const auth = await requireAdminRequest();
  if (!auth.ok) return auth.response;
  if (!canAccess(auth.ctx.role, "products", "PUT")) {
    return jsonError(403, "Forbidden", "forbidden");
  }
  const svc = requireServiceClient();
  if (!svc.ok) return svc.response;
  ...
  const parsed = parseJsonBody(raw, productUpdateSchema);
  ...
  const { data: before, error: fetchErr } = await supabase
    ...
    .maybeSingle();
  if (fetchErr) {
    console.error("[admin products PUT fetch]", fetchErr.message);
    return jsonError(500, fetchErr.message, "database");
  }
  if (!before) return jsonError(404, "Not found", "not_found");

  const { data, error } = await supabase.from("products").update(patch).eq("id", idParse.data).select("*").single();
  if (error) {
    console.error("[admin products PUT]", error.message);
    return jsonError(500, error.message, "database");
  }
...
export async function DELETE(request: Request, context: Ctx) {
  const auth = await requireAdminRequest();
  if (!auth.ok) return auth.response;
  if (!canAccess(auth.ctx.role, "products", "DELETE")) {
    return jsonError(403, "Forbidden", "forbidden");
  }
  const svc = requireServiceClient();
  if (!svc.ok) return svc.response;
  ...
  const { data: before, error: fetchErr } = await supabase
    ...
    .maybeSingle();
  if (fetchErr) {
    console.error("[admin products DELETE fetch]", fetchErr.message);
    return jsonError(500, fetchErr.message, "database");
  }
  if (!before) return jsonError(404, "Not found", "not_found");

  const { error } = await supabase.from("products").delete().eq("id", idParse.data);
  if (error) {
